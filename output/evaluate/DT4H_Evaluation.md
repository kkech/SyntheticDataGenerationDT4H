...
| aim50_eps1_seed0 | Cramer's V (cat-cat) | 406 | 0.0964 | 0.0521 | 0.7241 | 8 | `med_rasi|med_anti_coag_history` (0.0759 -> 0.812) |
| aim50_eps1_seed0 | corr-ratio (num-cat) | 1407 | 0.0394 | 0.0 | 0.8635 | 3 | `vital_signs_weight_value_p6mo_last|med_ll_history` (0.0114 -> 0.5705) |
...
| aim50_eps5_seed0 | Cramer's V (cat-cat) | 406 | 0.1611 | 0.091 | 0.5296 | 30 | `conditions_mc|conditions_pad` (0.0197 -> 0.9298) |
| aim50_eps5_seed0 | corr-ratio (num-cat) | 1407 | 0.0806 | 0.0 | 0.7534 | 50 | `lab_results_ntProBnp_value_first|conditions_mc` (0.0135 -> 0.694) |
...
| dpctgan_eps15_seed2 | corr-ratio (num-cat) | 4324 | 0.0456 | 0.022 | 0.8756 | 0 | `lab_results_valideGFR_value_last|ckd_severity_calculated_or_measured` (0.9616 -> 0.0) |
...
| dpctgan_eps1_seed0 | Cramer's V (cat-cat) | 9870 | 0.0931 | 0.0387 | 0.8054 | 0 | `cause_of_death_isRenal_f5a_w7d_first|cause_of_death_isNonRenalAndNonCV_f5a_w7d_first` (1.0 -> 0.0003) |
| dpctgan_eps1_seed0 | corr-ratio (num-cat) | 5264 | 0.0391 | 0.0205 | 0.909 | 0 | `lab_results_valideGFR_value_last|ckd_severity_categorizedValue` (0.9716 -> 0.035) |
...
| dpctgan_eps20_seed0 | Cramer's V (cat-cat) | 8515 | 0.0889 | 0.0408 | 0.8036 | 1 | `cause_of_death_isCV_f5a_w7d_first|cause_of_death_isNonRenalAndNonCV_f5a_w7d_first` (1.0 -> 0.0003) |
| dpctgan_eps20_seed0 | corr-ratio (num-cat) | 4324 | 0.0456 | 0.0237 | 0.8668 | 0 | `lab_results_valideGFR_value_last|ckd_severity_categorizedValue` (0.9716 -> 0.0187) |
...
| dpctgan_eps5_seed0 | Cramer's V (cat-cat) | 7021 | 0.1025 | 0.0442 | 0.7825 | 2 | `cause_of_death_isCV_f5a_w1a_first|cause_of_death_isNonRenalAndNonCV_f5a_w1a_first` (1.0 -> 0.0003) |
| dpctgan_eps5_seed0 | corr-ratio (num-cat) | 5076 | 0.0462 | 0.0249 | 0.8788 | 0 | `lab_results_valideGFR_value_last|ckd_severity_calculated_or_measured` (0.9616 -> 0.0) |
...
| dpctgan_eps8_seed0 | Cramer's V (cat-cat) | 7750 | 0.0999 | 0.0437 | 0.7857 | 0 | `cause_of_death_isNonRenalAndNonCV_f5a_w5a_first|cause_of_death_isAllCause_f5a_w5a_first` (1.0 -> 0.0004) |
| dpctgan_eps8_seed0 | corr-ratio (num-cat) | 5264 | 0.0454 | 0.0241 | 0.8672 | 0 | `eGFR_2021_ckd_epi_creatinine|ckd_severity_from_calculated_egfr` (0.9497 -> 0.0333) |
...
| mst_eps10_seed0 | Cramer's V (cat-cat) | 11175 | 0.2454 | 0.2013 | 0.2991 | 1667 | `conditions_heart_failure_hf_within_18mo_any|conditions_hf` (0.9533 -> 0.0055) |
| mst_eps10_seed0 | corr-ratio (num-cat) | 11280 | 0.1667 | 0.1012 | 0.4971 | 975 | `electrocardiographs_ecg_qrs_duration_pET_last|conditions_mi` (0.0328 -> 0.8458) |
...
| mst_eps15_seed0 | Cramer's V (cat-cat) | 11175 | 0.2428 | 0.2094 | 0.2862 | 1413 | `med_arni|med_antiarrhytmic_history` (0.033 -> 0.9833) |
| mst_eps15_seed0 | corr-ratio (num-cat) | 11468 | 0.1674 | 0.107 | 0.4868 | 994 | `encounter_primary_reason_number_of_days_to_rehosp_for_non_CV_f5a_first|encounter_primary_reason_non_CV_Disease_f5a_w1a_first` (0.8476 -> 0.0082) |
...
| mst_eps15_seed1 | Cramer's V (cat-cat) | 11175 | 0.2486 | 0.2114 | 0.2762 | 1582 | `conditions_heart_failure_hf_within_18mo_any|conditions_hf` (0.9533 -> 0.0027) |
| mst_eps15_seed1 | corr-ratio (num-cat) | 11468 | 0.1757 | 0.1129 | 0.4746 | 1096 | `smoking_status_smoker_startTime_count|encounter_primary_reason_renal_complications_f5a_w1a_first` (0.0343 -> 0.8611) |
...
| mst_eps15_seed2 | Cramer's V (cat-cat) | 11175 | 0.2546 | 0.2079 | 0.2745 | 1857 | `med_arb|med_inotropes_history` (0.0077 -> 0.9676) |
| mst_eps15_seed2 | corr-ratio (num-cat) | 11280 | 0.1755 | 0.1129 | 0.4765 | 1086 | `encounter_primary_reason_number_of_days_to_rehosp_for_non_CV_f5a_first|med_digitalis_history` (0.0335 -> 0.8418) |
...
| mst_eps1_seed0 | Cramer's V (cat-cat) | 11175 | 0.2901 | 0.2636 | 0.2479 | 2398 | `med_rdoad_syst_history|conditions_aidshiv` (0.0069 -> 1.0) |
| mst_eps1_seed0 | corr-ratio (num-cat) | 11092 | 0.1878 | 0.1052 | 0.4904 | 1081 | `lab_results_tropTHs_value_last|conditions_stroke` (0.0001 -> 1.9189) |
...
| mst_eps20_seed0 | Cramer's V (cat-cat) | 11175 | 0.2414 | 0.1858 | 0.3181 | 1725 | `med_arb|med_inotropes_history` (0.0077 -> 0.9613) |
| mst_eps20_seed0 | corr-ratio (num-cat) | 11468 | 0.1797 | 0.1137 | 0.4722 | 1134 | `encounter_primary_reason_number_of_days_to_rehosp_for_heart_failure_f5a_first|conditions_mi` (0.012 -> 0.8637) |
...
| mst_eps5_seed0 | Cramer's V (cat-cat) | 11175 | 0.2625 | 0.2283 | 0.2695 | 1822 | `conditions_heart_failure_hf_within_18mo_any|conditions_hf` (0.9533 -> 0.01) |
| mst_eps5_seed0 | corr-ratio (num-cat) | 11468 | 0.1558 | 0.0958 | 0.5092 | 754 | `encounter_primary_reason_number_of_days_to_rehosp_for_heart_failure_f5a_first|encounter_primary_reason_HF_Disease_f5a_w5a_first` (0.0 -> 0.9426) |
...
| mst_eps8_seed0 | Cramer's V (cat-cat) | 11175 | 0.2546 | 0.2068 | 0.2864 | 1844 | `conditions_heart_failure_hf_within_18mo_any|conditions_hf` (0.9533 -> 0.0015) |
| mst_eps8_seed0 | corr-ratio (num-cat) | 11468 | 0.1698 | 0.0994 | 0.5007 | 1046 | `encounter_primary_reason_number_of_days_to_rehosp_for_heart_failure_f5a_first|conditions_mi` (0.012 -> 0.8517) |
